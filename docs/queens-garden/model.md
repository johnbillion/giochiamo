...
count; player count only feeds `expansionsPerRound` (5 / 7 / 8 for 2 / 3 / 4).
...
```
Game
├─ Setup              (seed + deal supply; each player: empty expansion in centre slot)
├─ Round (×4)
│    ├─ Turn loop:    active players take turns; on your turn do ONE of
│    │                  { take tiles | take expansions | place expansions |
│    │                    place tiles on expansions | pass }
...
| Region | Fields | Notes |
|---|---|---|
| Supply / pools | **Tile bag** (remaining of 108) + **discard pile** + **expansion pool** (remaining of 36) + **seed** | bag-empty → shuffle discard back in (seed-driven, must be logged); batch draws & expansion picks seed-driven |
| Central area (per round) | **Pile** of this round's n expansions (n=5/7/8); the **top** expansion holds 4 tiles; **split-off expansions** each hold their leftover tiles (still draftable). Draft source = **all tiles in the central area**. A expansion is **takeable once emptied** (derived: tiles == 0). | top splits when <4 → next revealed w/ 4 fresh; leftovers discarded at round end |
| Scoring wheel | _(current position)_ | changes per round |
| Per player | **Play area** = 7 expansion slots (1 centre + 6 edge); centre starts with the **blank starter expansion** (6 free spaces, no identity), edges start empty. Each placed **non-centre expansion** carries a (colour, symbol) **identity tile** in 1 slot, leaving **5 free**. **Storage**: an **ordered tile area** (`StorageItem[]` of tiles + coins, ≤ **12**; starts with 3 coins; rearrangeable) + **2 expansions**. **Score**. **Passed-this-round** flag. | expansion identity is game-relevant (scoring); ≤ 6 + 6×5 = **36** placeable spaces; passed-flag stored |
...
- **Stored (irreducible):** the tile **bag**, **discard**, and **expansion pool** (the actual
...
- **Derived (never stored):** **status/phase** (`round > ROUND_COUNT ? GameOver : Playing`);
  whether a expansion is **takeable** (`open display tiles == 0`); **draftable attributes** and
...
|---|---|---|---|
| Take (draft) | chosen **colour**/**symbol** + which copy of each duplicated combo | ≥1 draftable item bears it; one instance per **distinct** matching combo, chosen & present; distinct tiles ≤ free tile storage (12) **and** expansions ≤ free expansion storage (2) | one chosen tile per distinct matching combo **and** all matching draftable expansions → storage; then resolve splits/reveals |
| Place expansion | stored expansion + target slot + **which slot its identity occupies** + **payment** | expansion in storage; target slot **empty**; the identity tile (on its chosen slot) shares a colour/symbol with **at least one** other-expansion tile it faces (a non-matching neighbour is fine if another matches; empty/no-neighbour fine); **payment valid** | a blank frame is placed with the identity tile on the chosen slot; payment discarded |
| Place tile | stored tile + target (slot, dir) + **payment** | tile in storage; target is an **empty space of a placed expansion**; placed tile shares a colour/symbol with **at least one** face-adjacent occupied tile (within-expansion ring + cross-edge), and joins no identical tiles in a run; **payment valid** | tile fills the space; payment discarded |
...
1. ~~Are garden expansions drafted & placed, or pre-set?~~ **RESOLVED:** drafted (`take expansions`)
   and placed (`place expansions`); centre slot starts with an empty expansion; the 6 edge slots
   start empty and need a expansion before tiles. *Remaining:* per-count expansion numbers.
2. ~~Does a garden expansion's centre symbol constrain placement/scoring?~~ **RESOLVED:** the
   centre symbol is **purely decorative** — no gameplay effect; not modelled.
3. ~~Expansion orientation — fixed or rotatable?~~ **RESOLVED:** rotatable — you choose **which
   slot the identity tile occupies** (6 choices). But we don't store rotation/identity: a placed
   expansion is just a **frame of 6 tile-slots** and the identity is a pre-placed tile on one of
   them (see #28). Placement legality is the tile-level adjacency rule applied to that tile.
4. ~~Does storage hold tiles + expansions; is it bounded?~~ **RESOLVED:** holds both; capped at
   **12 tiles and 2 expansions** (a precondition on the `take` actions).
5. ~~Adjacency across expansions — edge-only or corners too?~~ **RESOLVED:** **face/edge adjacency
   only** — corners don't count. The `dir ↔ dir+3` model in `garden.ts` is exactly right (one
   other-expansion neighbour per tile). *(A later "runs" rule handles sequences of tiles — separate.)*
...
8. ~~Placing a expansion — any slot, or only adjacent to an existing one?~~ **RESOLVED:** **any
   empty slot**, any rotation; no requirement to connect to an existing expansion (the identity may
   face empty garden space). Legality is the identity-adjacency rule (#25).
9. ~~Expansion supply / are expansions identical?~~ **RESOLVED:** exactly **36** expansions, one per
   ...
   draws n at random (n = 5/6/7). So a expansion IS game-relevant data (its colour + symbol).
10. ~~Take tiles vs. take expansions — separate or combined?~~ **RESOLVED:** **combined.** One draft
    action picks a **colour or a symbol** and takes **every** draftable tile *and* expansion
    bearing it (gated by both storage limits). _Slice follow-up: collapse the engine's
    `take-tiles`/`take-expansions` into one parameterised `draft` action in Pass 2._
11. ~~Split-off expansions + leftover tiles?~~ **RESOLVED:** a split expansion **carries its leftover
    tiles** (still draftable); the **expansion** becomes takeable once **all 4 tiles are drafted**.
...
14. ~~Split trigger?~~ **RESOLVED:** yes — fires the instant the top expansion drops below 4.
...
16. ~~"No identical tiles" nuance?~~ **RESOLVED:** you take **one of each distinct** matching
    ...
    chooses which physical copy**, because its location (top expansion vs a split-off) affects
    whether taking it empties/reveals a expansion. So a draft's tile count = number of *distinct*
    matching combos, and the action carries a **per-duplicate source choice**.
17. ~~Batch tiles vs. identity tile?~~ **RESOLVED:** the draftable tiles on a expansion are
    **separate** from the expansion's own immovable identity tile (and its placeable slots).
18. ~~Draft decisions (please confirm)?~~ **RESOLVED:**
    (a) a expansion emptied *by the current draft* becomes takeable **next** turn — only expansions
    already empty before the draft are taken with it;
    (b) a **expansion-only** draft (no matching tiles, just a matching emptied expansion) is allowed;
...
20. ~~Storage ordering?~~ **RESOLVED:** Both storage areas are ordered: the tile area is a `StorageItem[]`
    (`tile | coin`) and the expansion row is a `Expansion[]`. Order has **no gameplay effect**
    (`storageTiles`/`storageCoins` derive counts). One free **`reorder`** action targets either
    area (`area: 'tiles' | 'expansions'`) — current player, must be a permutation, **no turn cost**.
21. ~~Payment — a user must pay to place tiles and expansions from storage into their play area;
...
22. ~~Round end is automatic (condition-discovered)?~~ **RESOLVED:** When the pass that makes *everyone* passed
    ...
    **discards the central area's leftover tiles** and **deals a fresh pile** (n expansions + 4
...
23. ~~Placement assumptions~~ — **now implemented** ~~(flag if any are wrong):~~ (a) non-coin payment
    ...
    placed item ✓; (c) discard destinations — payment **tiles → discard pile**, **expansions & coins
...
24. ~~Garden topology + the rotation question (feeds placement).~~ The 7 expansion-slots form a
    ...
    position as **(slot 0–6, direction 0–5)**. Within a expansion, ring adjacency is `dir ± 1 mod
    ...
    **RESOLVED:** tile positions are indexed by **board direction**; a placed expansion is a
    **frame of 6 tile-slots** (the identity is just a pre-placed tile — see #28). Codified in
    ...
    `PlacedExpansion` / `Garden` / `SlotId` / `Direction` types in `types.ts`, all unit-tested.
    *The `garden` field is wired into `PlayerState` with the placement slice.*

25. ~~Per-player gardens + expansion-placement adjacency~~ — **RESOLVED:** gardens are **per-player**.
    A expansion goes in **any empty slot** at a chosen **rotation**; the **identity** must share a
    colour/symbol with **at least one** other-expansion tile it faces (or face only empty space) —
    see #26 for the adjacency predicate. **Face/edge adjacency only** (#5), so the identity has at
    most **one** other-expansion neighbour: `(neighbourSlot(slot, rot), rot+3)`. *(Tile-placement
...
26. ~~Tile placement = the generalized adjacency rule~~ — **CORRECTED** ~~(was: share exactly one
    with EVERY neighbour).~~ A tile goes on an **empty space of a placed expansion** (never a
    expansion-less area); legal iff it shares a colour/symbol (`sharesAttribute`) with **at least
    one** face-adjacent occupied tile — its expansion's ring neighbours (incl. the identity) and the
    ...
    catches them. Same predicate as #25; expansion placement is just the case where only the
...
27. ~~Runs — topology~~ **CORRECTED** ~~(supersedes the earlier fixed-lines model).~~ A *run* is a chain
    ...
    following the adjacency graph **freely** (rounding rings, crossing expansion edges); it is **not**
    ...
    **Earlier (now removed):** runs were modelled as contiguous arcs along 13 fixed lines (7 expansion
...
28. ~~Board = hex cells; identity is just a tile (simplification)~~ — **RESOLVED.** The screenshot
    ...
    cross-expansion neighbour — validating the degree-3 adjacency already in `garden.ts` (no rebuild).
    **Simplification:** `PlacedExpansion` is now just `{ tiles: (Tile|null)[6] }` — no stored
    ...
    `place expansion` = drop a blank frame + place the identity tile (same adjacency check as
...
29. ~~Placement~~ **codified.** `place expansion` / `place tile` (`placement.ts`): target valid (empty
    slot / empty space of a placed expansion) → shared adjacency rule (exactly-one attribute with
    ...
    placement, adjacency (match-one / mismatch), run duplicates, underpayment, expansion placement.

30. ~~Coins — earning~~ **RESOLVED** ~~(spending was #21).~~ A player earns coins by placing the tile that
    **completes a 6-tile region** (the sixth tile): **centre expansion → 1**, **a ring expansion → 3**,
    **a junction gap → 2** (the 6 holes where two adjacent ring expansions meet the centre, ringed by
    2 centre + 2+2 ring petals). Bonuses **stack** — one tile can complete several overlapping
    regions (a centre petal is shared by the centre expansion + 2 gaps → up to 1+2+2 = **5**). Only a
    **tile** placement can earn (a freshly placed expansion lays one tile, never a sixth), so coin
    logic lives only in `resolvePlaceTile`.
    - **Geometry resurrected (lean).** This re-needs the per-region position sets dropped in #27 —
      but **membership only, no cyclic order** (unlike the old run `LINES`). Expansions are trivial
